define([
	...
	'./vdomUtils',
	'dojo/_base/sniff'
], function (declare, lang, domConstruct, domClass, listen, has, h, List, miscUtil, vdomUtils) {
	function appendIfNode(parent, subNode) {
		if (subNode && subNode.vnodeSelector) {
	...
	var Grid = declare(List, {
		...
		createRowCells: function (tag, createCell, subRows, item, options) {
			...
			var rowNode = h('table.dgrid-row-table', { role: 'presentation' }),
				tr, trNode,
				si, sl, i, l, // iterators
				subRow, column, id, extraClasses, className,
				cellProperties,
				cellNode; // used inside loops
			...
			for (si = 0 , sl = subRows.length; si < sl; si++) {
				...
				for (i = 0 , l = subRow.length; i < l; i++) {
					...
					extraClasses = column.field ? '.field-' + replaceInvalidChars(column.field) : '';
					...
					cellProperties = {};
					cellProperties.columnId = id;
					if (column.colSpan) {
						cellProperties.colSpan = column.colSpan;
					}
					if (column.rowSpan) {
						cellProperties.rowSpan = column.rowSpan;
					}
					cellProperties.role = tag === 'th' ? 'columnheader' : 'gridcell';
					cellNode = h(tag + '.dgrid-cell' +
						(id ? '.dgrid-column-' + replaceInvalidChars(id) : '') + extraClasses, cellProperties);

					createCell(cellNode, column, item, options);
					// add the td to the tr at the end for better performance
					trNode.children.push(cellNode);
				}
			}
			return rowNode;
		},

		_createBodyRowCell: function (cellNode, column, item, options) {
			var cellData = item;
			...
			if (column.renderCell) {
				...
				// event handling, etc.
				appendIfNode(cellNode, column.renderCell(item, cellData, cellNode, options));
			}
			else {
				this._defaultRenderCell.call(column, item, cellData, cellNode, options);
			}
		},

		_createHeaderRowCell: function (cellNode, column) {
			var contentNode = column.headerNode = cellNode;
			var field = column.field;
			if (field) {
				cellNode.properties.field = field;
			}
			...
			if (column.renderHeaderCell) {
				...
				contentNode.text = 'label' in column ? column.label : column.field;
			}
			if (column.sortable !== false && field && field !== '_item') {
				cellNode.properties.sortable = true;
				vdomUtils.addClassToVNode(cellNode, 'dgrid-sortable');
			}
		},
		...
		_defaultRenderCell: function (object, value, cellNode) {
			// summary:
			...
			if (this.formatter) {
				...
				// NOTE: The follow is not a good long term solution because with this implementation,
				// the formatter can only return text; no HTML.
				cellNode.text = typeof formatter === 'string' && formatterScope ?
					formatterScope[formatter](value, object) : this.formatter(value, object);
			}
			else if (value != null) {
				// Maquette bug?  Setting cellNode.text to 0 cause nothing to be rendered.
				cellNode.text = String(value);
			}
		},

		renderRow: function (item, options) {
			var rowNode = this.createRowCells('td', lang.hitch(this, '_createBodyRowCell'),
			...
			return h('div', { role: 'row', key: item.id }, rowNode);
		},
		...
		renderHeader: function () {
			...
			headerNode.properties.role = 'row';

			// clear out existing header in case we're resetting
			headerNode.children = [];

			var rowNode = this.createRowCells('th', lang.hitch(this, '_createHeaderRowCell'),
				this.subRows && this.subRows.headerRows);
			this._rowIdToObject[rowNode.properties.id = this.id + '-header'] = this.columns;
			headerNode.children.push(rowNode);
		...
		resize: function () {
			...
			// Is it best practice with virtual DOMs to modify the DOM in cases like this?
			var headerTableNode = this.headerNode.children[0].domNode,
				contentNode = this.contentNode.domNode,
				width;
